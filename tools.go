...
func renderToolResult(name, body string, isErr bool, width int) string {
	...
	lines := strings.Split(strings.TrimRight(unwrapToolError(body), "\n"), "\n")
	hidden := 0
...
// unwrapToolError strips the <tool_use_error> wrapper the CLI puts around a
// failed tool's message. The card already says "error" and draws a red border,
// so the tag is markup the reader has to look past. The test is structural,
// like replay.go's: only a body that is *nothing but* the wrapper is unwrapped,
// so an error text that quotes the tag mid-sentence survives intact.
func unwrapToolError(body string) string {
	const openTag, closeTag = "<tool_use_error>", "</tool_use_error>"
	t := strings.TrimSpace(body)
	if !strings.HasPrefix(t, openTag) || !strings.HasSuffix(t, closeTag) {
		return body
	}
	inner := t[len(openTag) : len(t)-len(closeTag)]
