...
var palettes = map[string]palette{
	...
	// cinder is neutral chrome carrying a single orange accent, rather than a
	// spread of hues: the greys are the ash, the accent is the heat still in
	// it. The greys are hueless (0% saturation) on purpose — an earlier warm
	// set at hue 43° read as beige next to the ember rather than as ash, which
	// is the whole point of the theme.
	//
	// Three values are tuned rather than picked for looks, all for contrast
	// on #0F0F0F:
	//   mag  is a deeper burnt orange than the accent, because the lightbar
	//        prints white on it — white on the accent itself is 2.9:1.
	//   red  is pushed 25° toward crimson so an error never reads as the
	//        orange accent. (Gruvbox separates its orange and red by 21°.)
	//   gray is light enough to carry content at 4.3:1, not just hints; a
	//        dimmer grey suits a hint but this role holds real text, and on a
	//        flat status row it also holds every label.
	"cinder": {cyan: "#9E9E9E", mag: "#A83A1C", yel: "#E8853A", white: "#EDEDED", green: "#77A06E", red: "#DE5068", blue: "#808080", gray: "#787878", black: "#0F0F0F", accent: "#F05A28"},
}

// themeDef is one row in the /settings color-theme picker.
//
// banner is whether that theme shows the wordmark banner. Every theme states
// it, including the eleven that just want the default — that is what makes
// switching symmetric. An earlier version let a theme stay silent and only
// cinder spoke, so switching away from cinder left the banner hidden.
//
// This is visibility only. How the wordmark animates stays the user's setting
// (settings.Header) and no theme touches it.
//
// bars is the same idea for the inverted chrome (barfill.go): every theme
// states whether its status row and filename chip are filled, so switching in
// either direction restores the treatment the theme asked for.
...
var themes = []themeDef{
	{"bbs", "BBS (terminal ANSI)", "the original neon 16-color palette", true, barsFilled},
	{"dracula", "Dracula", "purple & pink on dark slate", true, barsFilled},
	{"nord", "Nord", "muted arctic blues", true, barsFilled},
	{"solarized", "Solarized Dark", "low-contrast teal & amber", true, barsFilled},
	{"tokyonight", "Tokyo Night", "soft neon on deep blue", true, barsFilled},
	{"gruvbox", "Gruvbox Dark", "warm retro earth tones", true, barsFilled},
	{"onedark", "One Dark", "Atom's classic dark", true, barsFilled},
	{"monokai", "Monokai", "the Sublime Text classic", true, barsFilled},
	{"catppuccin", "Catppuccin Mocha", "pastel on dark mauve", true, barsFilled},
	{"github", "GitHub Dark", "GitHub's dark mode", true, barsFilled},
	{"rosepine", "Rosé Pine", "muted rose & pine", true, barsFilled},
	// cinder is built for running inside another TUI that already shows its
	// own context bar, where the BBS banner is a redundant four rows. It is
	// the one theme that drops it, and the one theme with no filled bars.
	{"cinder", "Cinder", "graphite ash with one live ember, no banner", false, barsFlat},
}

// bannerFor returns the banner setting a theme asks for. An unknown id keeps
// the banner: a missing theme should not silently remove chrome.
func bannerFor(themeID string) string {
