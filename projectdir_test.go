...
// TestProjectSlugMatchesClaude pins the naming rule against real directories
// observed under ~/.claude/projects. Every one of these used to resolve to a
// path that does not exist, which is what emptied the resume picker and the
// replayed transcript for those projects.
func TestProjectSlugMatchesClaude(t *testing.T) {
	cases := []struct{ cwd, want string }{
		...
		{"/Users/w/Work/Triple Down/web", "-Users-w-Work-Triple-Down-web"},                         // space
		{"/Users/w/.config/wezterm", "-Users-w--config-wezterm"},                                   // dot: two dashes, no collapsing
		{"/Users/w/Work/BunkerIntel/fuel_forecaster", "-Users-w-Work-BunkerIntel-fuel-forecaster"}, // underscore
		{"/Users/w/Work/Tessio Provenance", "-Users-w-Work-Tessio-Provenance"},
	}
	for _, c := range cases {
...
func TestLoadPriorTranscriptSpacedCwd(t *testing.T) {
	...
	work := filepath.Join(t.TempDir(), "Triple Down", "web")
