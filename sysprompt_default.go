...
// and applied as claude's output style while /sysprompt is on. Kept in its own
// file because it is content, not logic — editing prose shouldn't mean
...
// directive. It's deliberately short: it rides in the system prompt of every
// request, so its cost is paid per turn, and the rules that survived are
