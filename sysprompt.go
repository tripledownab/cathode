...
//
// claude takes extra standing instructions as a *launch* flag. The text is the
// user's, this file owns the toggle, and outputstyle.go owns the delivery: the
// prompt is rendered into a claude output style and selected with
// --settings '{"outputStyle":"cathode"}'. It used to be appended with
// --append-system-prompt-file, which delivered the text but not the behaviour —
// read outputstyle.go for why, before changing the flag back.
//
// There is no runtime equivalent: the engine's intent switch accepts only turn,
// interrupt, set_model, set_permission_mode, set_max_thinking_tokens,
// apply_flag_settings and seed_read_state — none of which rebuild the system
// prompt. So toggling it has to restart the subprocess.
//
...
// the live session id, quit. The fresh process rewrites the style file, adds
// the flag, and resumes the same session. claude's context survives in full,
// but the visible transcript is replayed from the session log and capped at
// replayMax entries, so a long scrollback comes back shortened.
//
...
// sysPromptArgs is the launch flag pair that selects the user's instructions,
// or nil when the toggle is off or there's nothing to apply. It writes the
// style file as a side effect, so the launch and the file that launch names
...
func sysPromptItems(edited bool) []pickerItem {
	on := "apply it as claude's response style — " + sysPromptSummary()
	if edited {
		on = "edited since launch · select to reload — " + sysPromptSummary()
	}
	return []pickerItem{
		{id: sysPromptOn, title: "on", subtitle: on},
		{id: sysPromptOff, title: "off", subtitle: "run with claude's default system prompt and style"},
	}
}
