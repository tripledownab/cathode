...
func (m *model) resizeViewport() {
	// banner + divider (topChromeRows) + status(1) + prompt + the compact bar
	// when it's up
	vpH := m.h - 1 - m.topChromeRows() - m.promptRows() - m.compactRows()
	if vpH < 1 {
		vpH = 1
	}
...
// topChromeRows is how many rows the banner and its scene divider occupy: four
// normally (3 + 1), none when the banner is off. Hiding the banner takes the
// divider with it — a lone divider above the transcript labels nothing and
// just spends the row the setting was meant to reclaim.
//
// This is the single source of that number. view.go decides whether to emit
// those rows from the same condition, and the two must agree — a viewport
// budgeted for chrome that is not drawn leaves dead rows at the bottom of
// every frame.
//
// It reads settings.Banner, not the header animation: "off" as an animation
// still draws the banner, it just stops moving.
func (m *model) topChromeRows() int {
	if m.settings.Banner == bannerOff {
		return 0
	}
	return 4
}
