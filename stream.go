...
func (m *model) handleEvent(e Envelope) {
	...
	case "system":
		...
		case "init":
			...
			// The init line is where the launch flags are confirmed, so the
			// resolved output style is reported here too (outputstyle.go).
			seg, warn := initStyleNote(e.OutputStyle, m.settings.SysPrompt)
			m.add(entInfo, fmt.Sprintf("— session %s · %s%s —", short(e.Session), e.Model, seg))
			if warn != "" {
				m.add(entError, warn)
			}
		case "hook_response":
