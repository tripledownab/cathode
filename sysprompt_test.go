...
// would do something — selecting an output style claude cannot find is ignored
// in silence, so an enabled-but-empty toggle must not add it.
func TestSysPromptArgs(t *testing.T) {
	...
	if got[1] != outputStyleSettings() {
		t.Errorf("settings value = %q, want %q", got[1], outputStyleSettings())
	}
	// The style file has to exist by the time the flag names it, or claude
	// falls back to its default behaviour without saying so.
	b, err := os.ReadFile(outputStylePath())
	if err != nil {
		t.Fatalf("style file should be written before launch: %v", err)
	}
